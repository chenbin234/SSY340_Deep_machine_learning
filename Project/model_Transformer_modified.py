...
class EncoderLayer(nn.Module):
    def __init__(self, embedding_size, num_heads, feedforward_size):
...
#         self.dropout = nn.Dropout(dropout)
    ...
    def forward(self, x, mask):
        ...
        x = self.norm_output(x + attn_output)
...
#         self.dropout = nn.Dropout(dropout)
    ...
    def forward(self, x, encoder_output, source_mask, target_mask):
        ...
        feedforward_output = self.feed_forward(x)
        x = self.norm_feedforward_output(x + feedforward_output)
        return x
...
class Transformer(nn.Module):
    ...
    def __init__(self, encoder_input_size, decoder_input_size, embedding_size, num_heads, num_layers, feedforward_size):
        """class initializer
        ...
        self.encoder_layers = nn.ModuleList(
            [EncoderLayer(embedding_size, num_heads, feedforward_size) for _ in range(num_layers)])
        self.decoder_layers = nn.ModuleList(
            [DecoderLayer(embedding_size, num_heads, feedforward_size) for _ in range(num_layers)])
